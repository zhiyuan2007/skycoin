...
func (gw *Gateway) Spend(wltID string, password []byte, coins uint64, dest cipher.Address, msg string) (*coin.Transaction, error) {
	...
	gw.strand("Spend", func() {
		...
		tx, err = gw.vrpc.CreateAndSignTransaction(wltID, password, sv, unspent, gw.v.Blockchain.Time(), coins, dest, msg)
		if err != nil {
